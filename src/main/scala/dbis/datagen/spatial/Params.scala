...
object Params {
  ...
  def parseArgs(args: Array[String]): Params = {
    val parser = new OptionParser[Params]("Spatial Data Generator") {
      ...
      opt[Double]("min-x") action { (x,c) => c.copy(minX = x) } text("Minimal x value (for polygons for the center)")
      opt[Double]("max-x") action { (x,c) => c.copy(maxX = x) } text("Maximal x value (for polygons for the center)")
      opt[Double]("min-y") action { (x,c) => c.copy(minY = x) } text("Minimal y value (for polygons for the center)")
      opt[Double]("max-y") action { (x,c) => c.copy(maxY = x) } text("Maximal y value (for polygons for the center)")
